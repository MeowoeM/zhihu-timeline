...
// @version      0.1
// @description  niconiconi
// @author       You
// @match        https://www.zhihu.com/
// @include      https://www.zhihu.com/people/Meow0_o/following*
...
(function(){
    ...
    function getFollowingList(){
        ...
        if(typeof GM_getValue(storedFollowingListKey) == 'undefined'){
            console.log('no stored following list found, scrape a new one.')
            let followingList = getFollowingListFromScratch();
            return followingList
        }
        // update the existing following list
        else{
            console.log('updating following')
            let followingList = updateFollowingList()
    ...
    async function updateFollowingList(){
        var followingList = JSON.parse(GM_getValue(storedFollowingListKey));
        var latestFollowing = followingList[0];
        const followingPageList = await getFollowingPageList();
        var listToUpdate = []
        for(let i = 0; i < followingPageList.length; i++){
            var tmpFollowingList = await getFollowingThisPage(followingPageList[i], i);
            for(let j = 0; j < tmpFollowingList.length; j++){
                if(tmpFollowingList[j] == latestFollowing){
                    GM_setValue(storedFollowingListKey, JSON.stringify(listToUpdate.concat(followingList)));
                    console.log('update:' + listToUpdate);
                    return listToUpdate.concat(followingList)
                }
                else{
                    listToUpdate.push(tmpFollowingList[j]);
                };
            };
        };
        GM_setValue(storedFollowingListKey, JSON.stringify(listToUpdate.concat(followingList)));
    ...
    async function getFollowingListFromScratch(){
        const followingPageList = await getFollowingPageList();
